...
try {
    switch ($action) {
        ...
        case 'confirm':
            ...
            ob_end_clean();
            echo json_encode(['success' => true, 'message' => 'Match confirmed and email notification sent.']);
            break;
...
{
    $email         = !empty($match['owner_recovery_email'])
                    ? $match['owner_recovery_email']
                    : ($match['owner_cmu_email'] ?? null);
    $ownerName     = $match['owner_name']     ?? 'Student';
    $lostTitle     = $match['lost_title']     ?? 'your item';
    $foundLocation = $match['found_location'] ?? 'Campus';
    $confidence    = (int)($match['confidence_score'] ?? 0);

    if (!$email) {
        error_log('[process_match] No email address for match_id=' . $match['match_id']);
        return;
    }

    $sent = sendMatchNotificationEmail($email, $ownerName, $lostTitle, $foundLocation, $confidence);

    if (!$sent) {
        error_log('[process_match] Email failed for ' . $email . ' (match_id=' . $match['match_id'] . ')');
    }
}